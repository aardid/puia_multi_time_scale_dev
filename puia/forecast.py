...
class Forecast(object):
    ...
    def alert_model(self, threshold):      
        ...
        inds=np.where(self.y>=threshold)[0]# # to check, pre: inds=np.where(self.y>threshold)[0]
        if len(inds) == 0:
        ...
        # to check
        #am.true_negatives=int(am.total_time-am.false_positive)
        am.true_negative=int((am.total_time-am.inalert)/np.mean([am.alert_durations[i][0].days for i in range(len(am.alert_durations))]))
        #
        return am
    ...
    def fss(self, thresholds=None):
        if thresholds is None:
            thresholds=np.linspace(0,1,101)[::-1]
            thresholds=np.linspace(0.2,0.9,71)[::-1]
        else:
            thresholds=np.sort(thresholds)[::-1]
...
class FSS(object):
    ...
    def _setup(self, fcst):
        ...
        for j, am in enumerate(self.alert_models):
...
class ROC(object):
    ...
    def _setup(self, fcst):
        self.alert_models=[fcst.alert_model(th) for th in self.thresholds]
        atts=['fraction_inalert','inalert','true_positive','false_positive','false_negative','true_negative']
        for att in atts:
    ...
    def plot(self, save=None, reference=None):
        ...
        '''
        f,ax=plt.subplots(1,1)
        ax.plot(self.fraction_inalert*100, self.true_positive/self.number_eruptions*100, 'k-', label=f'ROC ({self.auc})')
        ...
        if False:
            plt.close()
            f,ax=plt.subplots(1,1)
            ax.plot((1.-self.fraction_inalert)*100, self.false_negative/self.forecast_days*100, 'k-', label=f'ROC ({self.auc})')
            ax.set_xlabel('time in warning [%]')
            ax.set_ylabel('non eruptions in warning [%]')
            plt.show()
            plt.close()
...
class MultiVolcanoROC(object):
    def __init__(self, rocs):
        ...
        #
        self.fraction_inalert=fraction_inalert
        self.true_positive=true_positive
        self.number_eruptions=total_eruptions
        self.percentage_true_positive=percentage_true_positive
        self.auc=auc
        self.thresholds=rocs[0].thresholds
        ## to check
        self.false_negative=np.sum([r.false_negative for r in self.rocs], axis=0)
        self.false_positive=np.sum([r.false_positive for r in self.rocs], axis=0)
        self.true_negative=np.sum([r.true_negative for r in self.rocs], axis=0)
        self.true_positive_rate=self.true_positive / (self.true_positive+self.false_negative)
        self.false_positive_rate=self.false_positive / (self.false_positive+self.true_negative)
        self.false_positive_rate[np.isnan(self.false_positive_rate)] = 0
        pass
        ##
